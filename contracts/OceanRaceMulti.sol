 ...
 */
contract OceanRaceMulti is FunctionsClient, ConfirmedOwner {
  ...
  function executeRequest(
    ...
    uint256 boatNumber = strToUint(args[2]);

    string memory args1 = position[players[boatNumber]];
    string memory args2 = direction[players[boatNumber]];
    string[] memory _args = new string[](3);
    _args[0] = args1;
    _args[1] = args2;
    _args[2] = args[2]; // boat number
    req.addArgs(_args);
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    if (nilErr) {
      ...
      string memory lastChar = getLastChar(result);

      uint256 boatNumber = strToUint(lastChar);

      updatePosition(players[boatNumber], result);
    }
  }
  ...
  function strToUint(string memory _str) public pure returns (uint256 res) {
    ...
    function getLastChar(string memory _originString) public view returns (string memory _lastChar){
        uint256 strLen = bytes(_originString).length - 1;
        bytes memory lastCharByte = new bytes(1);
        lastCharByte[0] = bytes(_originString)[strLen];
        return string(lastCharByte);
    }
