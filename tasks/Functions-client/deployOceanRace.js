...
task("oceanrace-deploy-client", "Deploys the OceanRace contract")
  .addOptionalParam("verify", "Set to true to verify client contract", false, types.boolean)
  .setAction(async (taskArgs) => {
    if (network.name === "hardhat") {
      throw Error(
        'This command cannot be used on a local hardhat chain.  Specify a valid network or simulate an OceanRace request locally with "npx hardhat functions-simulate".'
      )
    }

    console.log(`Deploying OceanRace contract to ${network.name}`)
    ...
    console.log(`\nOceanRace contract deployed to ${clientContract.address} on ${network.name}`)
  })
