...
console.log(
  `HTTP GET Request to ${url}?latitude=${newPositionLatitude.toFixed(2)}&longitude=${newPositionLongitude.toFixed(
    2
...
function getNewPosition(objP1, objP2) {
  ...
  const _newPositionLongitude = ((λ3 * 180) / Math.PI + 360) % 360 // in degrees

  //The longitude can be normalised to −180…+180 using (lon+540)%360-180
  //const newPositionLongitude = (_newPositionLongitude+540)/360-180;
  
  let newPositionLongitude = _newPositionLongitude;
  if (_newPositionLongitude>180){
    newPositionLongitude = _newPositionLongitude-360
  }


  //console.log(`new position: ${φ3} : ${λ3}`);
